...
void Circle::draw(GraphicsContext* gc, viewcontext* vc)
{
	...
	Vertex temp;
	temp.x = (int)floor(midpoint.x+0.5);
	temp.y = (int)floor(midpoint.y+0.5);
	int radiusI = (int)floor((radius*vc->getZoom())+0.5);

	temp = vc->modelToDevice(temp);

	gc->setColor(midpoint.color);
	gc->drawCircle(temp.x,temp.y,radiusI);

	if(fill)
	{
		// See fill.cpp for implementation of fill
		// Can also be seen at bottom of gcontext.cpp
		gc->fFill(temp.x,temp.y,gc->getPixel(temp.x,temp.y));
	}
}
