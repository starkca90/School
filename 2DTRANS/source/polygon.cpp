 ...
 */
void Polygon::draw(GraphicsContext* gc, viewcontext* vc)
{
	Vertex p[points];
	for(int i=0; i < points; i++)
	{
		p[i].x = (int)floor(verticies[i].x+0.5);
		p[i].y = (int)floor(verticies[i].y+0.5);
		p[i] = vc->modelToDevice(p[i]);
	}

	gc->setColor(verticies[0].color);
	for(int i=0; i < (points-1); i++)
	{
		gc->drawLine(p[i].x,p[i].y,p[i+1].x,p[i+1].y);
	}
	gc->drawLine(p[points-1].x,p[points-1].y,p[0].x,p[0].y);
